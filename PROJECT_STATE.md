...
| **`spmm_max_csr`** (NEW op) | **native (Metal)** | Max-reducing SpMM returning `(out, arg)` where `arg[i,f]`=winning source node `col[e*]` (first on ties, sentinel `x.size(0)` for empty rows) so a GraphSAGE-max layer's backward routes grad to that neighbor (`mps/spmm_metal.mm`). Atomic-free per-row Metal (f32/f16/bf16) + CPU. Enables fused GraphSAGE-max on MPS. |
